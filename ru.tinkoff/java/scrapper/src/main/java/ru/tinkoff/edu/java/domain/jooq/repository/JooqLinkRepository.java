...
public class JooqLinkRepository implements LinkRepository {
    ...
    private final long minutesToCheckInMillis = (long) minutesToCheck * 60 * 1000;
    ...
    public List<LinkModel> readAllToUpdate() {
        var cutoffTime = new Timestamp(System.currentTimeMillis() - minutesToCheckInMillis);
        return context
            .select(link)
            ...
            .stream()
            .map(r -> new LinkModel(
                r.getValue(link.LINK_ID_PK),
                r.getValue(link.CHAT_ID),
                URI.create(r.getValue(link.URI)),
                r.getValue(link.LAST_CHECKED_AT)
                    .atOffset(ZoneOffset.UTC),
                r.getValue(link.LAST_ACTIVITY_AT)
                    .atOffset(ZoneOffset.UTC),
                r.getValue(link.ISSUE_COUNT),
                r.getValue(link.ANSWER_COUNT)
            ))
            .toList();
    }
